...
let getConfig = exports.getConfig = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (projectDir, configPath, configFromOptions) {
        let packageMetadata = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : new (_lazyVal || _load_lazyVal()).Lazy(function () {
        ...
        if (parentConfig.files != null && config.files != null && (Array.isArray(config.files) || typeof config.files === "string") && Array.isArray(parentConfig.files) && parentConfig.files.length > 0) {
            ...
            if (typeof mainFileSet === "object" && (mainFileSet.from == null || mainFileSet.from === ".")) {
                ...
                (_mainFileSet$filter = mainFileSet.filter).push.apply(_mainFileSet$filter, _toConsumableArray((0, (_builderUtil || _load_builderUtil()).asArray)(config.files)));
                delete config.files;
            }
        }
...
let validateConfig = exports.validateConfig = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (config) {
        ...
        yield (0, (_readConfigFile || _load_readConfigFile()).validateConfig)(config, schemeDataPromise, function (message, errors) {
            if ((_builderUtil || _load_builderUtil()).debug.enabled) {
                (0, (_builderUtil || _load_builderUtil()).debug)(JSON.stringify(errors, null, 2));
            }
            return `${message}
...
`;
    ...
    return function validateConfig(_x5) {
