...
class FileMatcher {
    ...
    computeParsedPatterns(result, fromDir) {
        ...
        for (let pattern of this.patterns) {
            ...
            if (!(pattern.indexOf(".") !== -1) && !(0, (_filter || _load_filter()).hasMagic)(parsedPattern)) {
...
function getMainFileMatchers(appDir, destination, macroExpander, platformSpecificBuildOptions, packager, outDir, isElectronCompile) {
    ...
    if (!matcher.isSpecifiedAsEmptyArray && (matcher.isEmpty() || matcher.containsOnlyIgnore())) {
        ...
        if (!(patterns.indexOf("package.json") !== -1)) {
    ...
    patterns.splice.apply(patterns, [insertIndex, 0].concat(customFirstPatterns));
    ...
    patterns.push("!**/{.git,.hg,.svn,CVS,RCS,SCCS," + "__pycache__,.DS_Store,thumbs.db,.gitignore,.gitkeep,.gitattributes,.npmignore," + ".idea,.vs,.flowconfig,.jshintrc,.eslintrc," + ".yarn-integrity,.yarn-metadata.json,yarn-error.log,yarn.lock,package-lock.json,npm-debug.log," + "appveyor.yml,.travis.yml,circle.yml,.nyc_output}");
...
function copyFiles(matchers) {
    ...
    return (_bluebirdLst2 || _load_bluebirdLst2()).default.map(matchers, (() => {
        var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (matcher) {
            ...
            return yield (0, (_fs || _load_fs()).copyDir)(matcher.from, matcher.to, matcher.createFilter());
        });
